...
import static org.lmdbjava.KeyRange.*;
...
public final class TutorialTest {
    ...
    public void tutorial4() throws IOException {
        ...
        try (Txn<ByteBuffer> txn = env.txnWrite()) {
            ...
            key.putInt(1).flip();
            final KeyRange<ByteBuffer> range = atLeast(key);
        ...
        try (Txn<ByteBuffer> txn = env.txnRead()) {
            ByteBuffer atLeast = allocateDirect(4).putInt(1);
            atLeast.flip();
            try (CursorIterator<ByteBuffer> it = db.iterate(txn, atLeast(atLeast))) {
                for (final KeyVal<ByteBuffer> kv : it.iterable()) {
                    assertThat(kv.key(), notNullValue());
                    assertThat(kv.val(), notNullValue());
                }
        ...
        try (Txn<ByteBuffer> txn = env.txnRead()) {
            ByteBuffer from = allocateDirect(4).putInt(1);
            from.flip();

            ByteBuffer to = allocateDirect(4).putInt(1);
            to.flip();

            try (CursorIterator<ByteBuffer> it = db.iterate(txn, closed(from, to))) {
                for (final KeyVal<ByteBuffer> kv : it.iterable()) {
                    assertThat(kv.key(), notNullValue());
                    assertThat(kv.val(), notNullValue());
                }
