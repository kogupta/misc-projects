...
object Server {
  ...
  val reframedFlow: Flow[ByteString, String, NotUsed] =
    Framing.delimiter(delimiter, 100).map(_.utf8String)
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] = {
    def statefulFlatMap: String => List[Identity] = {
      var maybeId: Option[Identity] = None
      s => {
        maybeId match {
          case Some(id) => id :: Nil
          case None =>
            maybeId = Identity.tryParse(s)
            Identity.tryParse(s).toList
        }
      }
    }

    reframedFlow
      .statefulMapConcat(() => statefulFlatMap)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] = {
    ...
    type Acc = SortedSet[Event]
    type Buf = mutable.SortedSet[Event]

    def findNext(expected: Int, buffer: Buf): (Int, Acc, Buf) = {
      buffer.foldLeft((expected, SortedSet.empty, buffer)) {
        case (triple@(n, acc, buf), event) =>
          if (n == event.sequenceNr) (n + 1, acc + event, buf -= event)
          else triple
      }
    }

    def statefulFlatMap: Event => Set[Event] = {
      var buffer = mutable.SortedSet[Event]()
      var nextExpected = 1
    ...
    Flow[Event].statefulMapConcat(() => statefulFlatMap)
  }
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] = {
    def fn: Event => Set[(Event, Followers)] = {
      ...
      event => {
        followers = event match {
          case Follow(_, from, to) =>
            val fs: Set[Int] = followers.get(from)
              .map(_ + to)
              .getOrElse(Set(to))
            followers.updated(from, fs)

          case Unfollow(_, from, to) =>
            val fs = followers.get(from)
              .map(_ - to)
              .getOrElse(Set.empty)
            followers.updated(from, fs)
    ...
    Flow[Event].statefulMapConcat(() => fn)
  }
